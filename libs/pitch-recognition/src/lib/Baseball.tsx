...
import { Mesh, MeshStandardMaterial, Group, BufferGeometry } from 'three';
...
export default function Model({ ...props }: JSX.IntrinsicElements['group']) {
  ...
  const { nodes, materials } = drei.useGLTF('assets/pitch-recognition/baseball.glb');
  const { pitch, isRightHandedPitcher, rotationsPerSecond } = useContext(BaseballContext);
  ...
  function getMesh(nodes: any): Mesh {
    return nodes['mesh_0'] as Mesh
  }

  function getBufferGeometry(mesh: Mesh): BufferGeometry {
    return mesh['geometry'] as BufferGeometry
  } 
  return (
    
    <group ref={group} {...props} dispose={null}>
          <mesh geometry={getBufferGeometry(getMesh(nodes))} material={materials['baseball_01']} />
    </group>
  )
}

drei.useGLTF.preload('assets/pitch-recognition/baseball.glb')